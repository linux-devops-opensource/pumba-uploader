...
const storageApiUrl = process.env.STORAGE_MANAGER_URL;

// const storageApiUrl: string = 'http://20.73.218.20:3000';
const nexusUrl = process.env.NEXUS_BASE_URL;
...
export class PackageController {
	...
	async handlePackage(packageName: string, session_id: string, localFilesLocation: string) {
		try {
			await this.downloadPackageLocally(
				`${storageApiUrl}/sessions/${session_id}/file/${packageName}`,
				`${localFilesLocation}/${packageName}`
			);
	...
	async uploadSinglePackage(assetName: string, session_id: string) {
		...
		let repoName = 'npm-local-test';

		let url = nexusUrl + `/service/rest/v1/components?repository=${repoName}`;

		try {
			console.log(fs.createReadStream(`${tempFileLocation}/${session_id}/${assetName}`));
			const data = {
